...
export function stringifySingle(object: any) {
  if (object.stack) {
    // 에러 객체
    return object.stack;
...
export function stringify(objects: any[]) {
  return objects.map(stringifySingle).join(' ');
}

export function formatLog(messages: any[]) {
  ...
  const payload = stringify(messages);
